  ...
  if (node != nullptr)
	{
		...
		if(node->GetNome() == food) found = node;
		SearchInOrderInternal(node->GetRight(), food, found);
	}
}

NodeAVL* AVL::Insert(int id, const std::string& data, int quantidade,float valor)
{
	if (IsEmpty())
	{
		m_Root = new NodeAVL(id, data,quantidade,valor);
		//m_Root->UpdateBalanceFactor();
		return m_Root;
	}

	return InsertInternal(m_Root, nullptr, id, data,quantidade,valor);
}

NodeAVL* AVL::InsertInternal(NodeAVL* node, NodeAVL* parent, int id, const std::string& data,int quantidade,float valor)
{
	if (node == nullptr)
		node = new NodeAVL(id, data,quantidade,valor, parent);
	else if (node->GetID() == id)
		return nullptr; // Error! Cannot insert duplicate.
	else if (node->GetID() > id)
		node->SetLeft(InsertInternal(node->GetLeft(), node, id, data,quantidade,valor));
	else if (node->GetID() < id)
		node->SetRight(InsertInternal(node->GetRight(), node, id, data,quantidade,valor));
...
float AVL::SomaProdutos() const
{
	return SomaProdutosInternal(m_Root,0);
}

float AVL::SomaProdutosInternal(NodeAVL* node,float Soma) const
{
	if (node != nullptr)
	{
		Soma=Soma+SomaProdutosInternal(node->GetLeft(),0);
		Soma=Soma+ (node->GetQtde()*node->GetValor());
		Soma=Soma+SomaProdutosInternal(node->GetRight(),0);
		return Soma;
	}
	return 0;
}
std::string AVL::Exer5(int quant) const
{
	return Exer5Internal(m_Root,quant);
}

std::string AVL::Exer5Internal(NodeAVL* node,int quant) const
{
	if (node != nullptr )
	{
		std::ostringstream oss;
		oss << Exer5Internal(node->GetLeft(),quant);
		if(node->GetQtde()<quant){
		oss << node->ToString();
	}
		oss << Exer5Internal(node->GetRight(),quant);
		return oss.str();
	}
